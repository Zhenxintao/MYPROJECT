    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        for (ForecastSourceCore forecastSourceCore : list) {
            ...
            if (StringUtils.isBlank(dispatch)) {
                // 进行24小时预测
                foreCastHandler.forecastHour(formatTime, forecastRequest, forecastWeatherCoef,forecastWebPageConfig);
            } else {
                List<Dispatch> dispatchList = JSONObject.parseArray(dispatch, Dispatch.class);
                if (!CollectionUtils.isEmpty(dispatchList)) {
                    // 进行阶段预测
                    foreCastHandler.forecastStageHour(formatTime, forecastRequest, forecastWeatherCoef,forecastWebPageConfig);
                }
